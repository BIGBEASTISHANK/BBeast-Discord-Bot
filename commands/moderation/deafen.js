...
=======
        if(!args[0]) return message.channel.send({ embed: { color: `RANDOM`, description: "Please Tag the person to deafen!" } })
...
=======
            message.channel.send({ embed: { color: `RANDOM`, description: `${member} was deafened. **Reason**: ${reason}` } })
        } catch (err) {
            console.log(err)
